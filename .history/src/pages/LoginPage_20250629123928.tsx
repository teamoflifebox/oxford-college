...
//           animate={{ opacity: 1, y: 0 }}
//           transition={{ duration: 0.6 }}
//           className="card rounded-2xl shadow-lg bg-white/80 backdrop-blur-md px-6 py-8"
//         >
//           {/* Header */}
...
const LoginPage: React.FC = () => {
  ...
  const getPlaceholderText = () =>
    formData.role === 'student'
      ? 'Enter roll number (e.g. CS2023001)'
      : 'Enter job ID (e.g. FAC001)';

  const getIdentifierIcon = () =>
    formData.role === 'student' ? Hash : Briefcase;

  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-primary-50 flex items-center justify-center px-4 py-8 relative">
      {/* Background Effects */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-primary-200/20 rounded-full blur-3xl -translate-y-2/3 translate-x-1/3" />
      <div className="absolute bottom-0 left-0 w-72 h-72 bg-accent-100/30 rounded-full blur-2xl translate-y-1/2 -translate-x-1/2" />

      <motion.div
        initial={{ opacity: 0, y: 40 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6 }}
        className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 z-10"
      >
        {/* Header */}
        <div className="text-center mb-6">
          <button
            onClick={() => navigate('/')}
            className="inline-flex items-center gap-2 text-sm text-neutral-500 hover:text-primary-600 mb-4"
          >
          ...
          <div className="flex justify-center mb-2">
            <div className="bg-gradient-to-tr from-primary-500 to-accent-500 p-3 rounded-xl shadow-md">
              <GraduationCap className="w-7 h-7 text-white" />
            </div>
          </div>

          <h1 className="text-2xl font-bold text-primary-700">Oxford ERP</h1>
          <p className="text-sm text-neutral-500">Technology Education Portal</p>
          <div className="flex justify-center items-center gap-2 text-xs text-neutral-400 mt-1">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="Enter your password"
                className="w-full border border-neutral-300 rounded-lg px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300"
                required
              />
              <button
                type="button"
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-primary-600 hover:bg-primary-700 text-white rounded-lg py-2 font-semibold text-sm transition duration-200"
          >
            {isLoading ? 'Signing In...' : 'Sign In to ERP'}
          </button>
        </form>
        ...
        <footer className="mt-6 text-center text-xs text-neutral-400">
          <p className="mb-1 flex items-center justify-center gap-2">
            <GraduationCap className="w-4 h-4 text-primary-500" />
            Powered by <strong className="text-primary-600">Lifebox NextGen Pvt Ltd</strong>
          </p>
          <div className="flex justify-center gap-3">
