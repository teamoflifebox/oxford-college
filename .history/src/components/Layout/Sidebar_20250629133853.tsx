...
interface SidebarProps {
  /** `true` → drawer visible on < md; always visible ≥ md */
  isOpen?: boolean;
  /** closes the drawer (called when backdrop is clicked or after a nav) */
  onClose?: () => void;
}
    ...
    <>
      ...
      <div
        className={`fixed inset-0 bg-black/25 backdrop-blur-sm transition-opacity
                    md:hidden z-40 ${isOpen ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
        onClick={onClose}
      />

      {/* ===== SIDEBAR (drawer on mobile) ===== */}
      <aside
        className={`fixed top-0 left-0 w-64 h-screen pt-[--nav-h] bg-white/90 backdrop-blur-md
                    border-r border-neutral-200/50 shadow-soft overflow-y-auto
